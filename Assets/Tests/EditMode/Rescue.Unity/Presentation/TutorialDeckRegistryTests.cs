...
namespace Rescue.Unity.Presentation.Tests
{
    public sealed class TutorialDeckRegistryTests
    {
        ...
        [Test]
        public void TutorialDeckRegistry_L00SecondCardTeachesTwoOrMoreBeforeWater()
        {
            ...
            TutorialCard card = deck.cards[1];
            Assert.That(card.title, Is.EqualTo("TAP TWO OR MORE"));
            Assert.That(
                card.body,
                Is.EqualTo("Tap any connected group of 2 or more matching supplies to clear it.\n\nThe water only rises when you move, so look as long as you need."));
            Assert.That(
        ...
        [Test]
        public void TutorialDeckRegistry_L00ThirdCardTeachesRisingWaterLevels()
        {
            bool found = TutorialDeckRegistry.TryGetDeck("L00", TutorialShowTiming.Pre, out TutorialDeck deck);
            ...
            Assert.That(card.title, Is.EqualTo("RISING WATER LEVELS"));
            Assert.That(
                card.body,
                Is.EqualTo("The light blue shallow water shows where it will flood next and rises each turn.\n\nFlooded rows are already lost."));
            Assert.That(
                card.imageResourcePath,
        ...
        [Test]
        public void TutorialDeckRegistry_L01SecondCardTeachesCrateNeighborClears()
        {
            ...
            Assert.That(
                card.imageResourcePath,
                Is.EqualTo("Rescue.Unity/UI/Tutorial/crate_neighbor"));
        }
