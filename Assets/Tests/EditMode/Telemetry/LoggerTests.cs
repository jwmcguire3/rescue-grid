...
namespace Rescue.Telemetry.Tests
{
    public sealed class LoggerTests
    {
        ...
        private static IEnumerable<ITelemetryEvent> AllEventTypes()
        {
            ...
            yield return new WaterForecastEvent("L1", 75, 4, WaterContactMode.OneTickGrace.ToString(), 2, true, 1);
            yield return new WaterRiseEvent("L1", 80, 4, 2);
            yield return new VineGrowthEvent("L1", 90, 5, coord);
            yield return new VinePreviewEvent("L1", 88, 5, coord);
            yield return new UndoUsedEvent("L1", 110, 3);
            yield return new TargetExtractedEvent("L1", 120, 4, "t0");
            yield return new TargetStateTransitionEvent("L1", 118, 4, "t0", coord, TargetReadiness.OneClearAway, TargetReadiness.ExtractableLatched);
            yield return new FinalRescueEvent("L1", 122, 4, "t0", true, true);
            yield return new FinalRescueDockOverflowOverrideEvent("L1", 123, 4, 1);
            yield return new HazardAdvanceSkippedEvent("L1", 124, 4, "final_rescue", true, true);
            yield return new TargetLostEvent("L1", 130, 6, "t1");
            yield return new TargetDistressedEvent("L1", 135, 6, "t1", "entered");
            yield return new GraceEvent("L1", 136, 6, "t1", "entered");
            yield return new AssistedSpawnEvent("L1", 137, 6, "dock_pressure", "dock=5/7", 2, true, true, 0.9d);
            yield return new AssistedSpawnFollowUpEvent("L1", 138, 6, 8, DebrisType.A);
            yield return new DeadboardLikeStateEvent("L1", 139, 9, "no_valid_groups");
